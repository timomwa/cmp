...
public class StatsAction extends BaseActionBean {
	...
	public Resolution getStats() throws JSONException{
		Query qry = cmp_dao.resource_bean.getEM().createNativeQuery("select date(timeStamp) dt, count(*) count, price, sum(price) total_kshs from  billable_queue where success=1 group by dt order by dt desc limit 30");
